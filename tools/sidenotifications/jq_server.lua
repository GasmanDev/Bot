...
addEventHandler("onPlayerJoin", root,
	function()
		outputSideChat(""..string.gsub(getPlayerName(source), '#%x%x%x%x%x%x', '' ).." đã xuất hiện",root,255,255,255)
	end
)

addEventHandler("onPlayerLogin", root,
	function()
		outputSideChat(""..string.gsub(getPlayerName(source), '#%x%x%x%x%x%x', '' ).." đã tham gia",root,255,255,255)
	end
)

addEventHandler("onPlayerQuit",root,
	function()
		outputSideChat("Chúng ta đã mất "..string.gsub(getPlayerName(source), '#%x%x%x%x%x%x', '' ).. "",root,255,255,255)
	end
)
