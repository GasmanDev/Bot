...
local maxLines = 8
local oocMessages = {}
local bitstream = dxCreateFont(":DayZ/fonts/bitstream.ttf", 12)
...
local helpFont = dxCreateFont(":DayZ/fonts/bitstream.ttf", 20)
local sWidth,sHeight = guiGetScreenSize() -- The variables
--1280x800
function removeHelpScreen()
	setTimer (
		function()
			removeEventHandler("onClientRender", root, forceFeedHelp)
		end, 5000, 1 )
end	
addEvent("ForceFeedHelpMessage",true)

function forceFeedHelp()
	dxDrawText("         [F9]\nÐể xem hướng dẫn", sWidth*(587/1280), sHeight*(239/800), sWidth*(696/1280), sHeight*(276/800), tocolor(255, 255, 255, 255), 1.00, helpFont, "left", "top", false, false, false, false, false)
end
